...
function HostEleventh() {
	...
	const { amenitiesValue, favoritesValue, safetyitemsValue } = useSelector((state) => state.sixthpage);
	const { imageValue } = useSelector((state) => state.seventhpage);

	const { titleValue, descriptionValue } = useSelector((state) => state.ninthpage);
				...
				<Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
					<Grid item xs={6}>
						...
						>
							...
							>
								<Image style={{ width: 950, height: 800 }} src={imageValue} alt="no image" />
							</Container>
						</Grid>
					</Grid>
					<Grid item xs={6}>
						...
						>
							...
							<div style={{ fontSize: 25 }}>
								<p>
									Guest: {guestValue}, Beds: {bedsValue}, BedRooms: {bedroomsValue}, Bathrooms:{' '}
									{bathroomsValue}
								</p>
							</div>
